...
router.get("/search", async (req, res) => {
  try {
    ...
    const cars = await Car.find({
      $or: [
        //comparison operator   $ is the syntax used
    ...
    const message =
      cars.length === 0 ? "No cars found matching your search." : null;
...
router.get("/:id", ensureSignedIn, async (req, res) => {
  ...
  const isFavorited = car.favoritedBy.some((userId) =>
    userId.equals(req.user?._id)
  );
  owner = req.body.owner;
  res.render("cars/show.ejs", {
    title: `${car.model} in ${car.city}`,
    car,
    isFavorited,
    owner,
  });
